...
if os.path.isdir(rootDir) and os.path.exists(rootDir):
    ...
    if not includeHidden:
        excluded_filter.appen(".*")
    if not includeSystem:
        excluded_filter += system_file_names
    print("\dirtree{%")
    for dirName, subdirList, fileList in sorted(filtered_walk(rootDir, depth=maxDepth, excluded_dirs=excluded_filter, excluded_files=excluded_filter)):
